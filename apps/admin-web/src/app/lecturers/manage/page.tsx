    ...
    <AdminShell>
      <div className="space-y-6">
        <div className="flex items-start justify-between">
          ...
          <Button className="bg-red-600 text-white hover:bg-red-700" onClick={() => setShowInvite((v) => !v)}>
            + Invite Lecturer
          </Button>
        </div>
        ...
        {showInvite && (
          <InviteUserModal
            ...
            onInvited={(user) => setLecturers((list) => [user, ...list])}
          />
        )}
              ...
              <table className="min-w-full table-fixed">
                <thead className="sticky top-0 z-10 bg-white">
                  <tr className="border-b border-zinc-200">
                    <th className="w-1/3 px-4 py-3 text-left text-xs font-semibold text-zinc-600">Lecturer</th>
                    <th className="w-24 px-4 py-3 text-left text-xs font-semibold text-zinc-600">Students</th>
        ...
        {editingUser && (
          <EditUserModal
            ...
            user={editingUser}
            onUpdated={(updated) => setLecturers((list) => list.map((u) => (u.id === updated.id ? updated : u)))}
          />
        )}
      </div>
    </AdminShell>
