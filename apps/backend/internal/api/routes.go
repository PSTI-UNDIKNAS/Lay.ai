...
func SetupRoutes(db *pgxpool.Pool) *gin.Engine {
	...
	v1 := router.Group("/api")
	{
		...
		learningUnits.Use(middleware.AuthWithStatusMiddleware(authService))
		{
			...
			learningUnits.GET("/courses/:courseId/units/summary", middleware.EnrollmentOrCourseOwnerMiddleware(authService, enrollmentStore, courseService), learningUnitHandler.GetLearningUnitSummaries)
			...
			learningUnits.GET("/:unitId/assignments", learningUnitHandler.GetAssignments)
			...
			learningUnits.GET("/:unitId/quizzes", learningUnitHandler.GetQuizzes)
			...
			learningUnits.GET("/:unitId/materials", learningUnitHandler.GetMaterials)
			learningUnits.GET("/materials/:documentId/download", learningUnitHandler.DownloadMaterial)
