...
export default function AiChatbotPage() {
  ...
  const courses = [
    {
      id: 'c1',
      name: 'Sistem Informasi Manajemen Ekowisata Konservasi Mangrove Sungai Pinang',
      topics: 4,
    },
  ...
  const topicsByCourse: Record<
    string,
    Array<{ id: string; name: string; courseId: string; courseName: string }>
  > = {
    c1: [
      {
        id: 'c1-t1',
        name: 'Introduction to Mangrove Conservation',
        courseId: 'c1',
  ...
  const availableTopics = selectedCourseIds.flatMap(
    (id) => topicsByCourse[id] ?? [],
  );
  ...
  return (
    <LecturerShell>
      <div className="flex h-full flex-col space-y-6">
        <div className="grid flex-1 grid-cols-1 gap-6 lg:grid-cols-[1fr_24rem]">
          <div className="flex h-full flex-col space-y-4">
            ...
            <Card className="flex-1 p-0" bodyClassName="h-full">
                ...
                <div className="flex-1 space-y-4 overflow-y-auto p-4">
                  <div className="flex items-end gap-2">
                    ...
                    <div className="max-w-[75%] rounded-2xl bg-zinc-100 p-3 text-sm text-zinc-900">
                      Sumber tersebut menyajikan artikel jurnal ilmiah yang
                      berfokus pada perancangan Sistem Informasi Manajemen (SIM)
                      Ekowisata melalui pendekatan konservasi mangrove di Nagari
                      Sungai Pinang, Kabupaten Pesisir Selatan. Penelitian ini
                      bertujuan untuk mengatasi berbagai kendala, seperti
                      kurangnya akses informasi dan masalah dalam manajemen
                      penjualan bibit serta pelaporan perkembangan mangrove,
                      dengan menciptakan sebuah aplikasi berbasis web.
                    </div>
                  </div>
          ...
          <div className="fixed inset-0 z-50">
            ...
            <div className="absolute left-1/2 top-20 w-full max-w-3xl -translate-x-1/2 rounded-xl bg-white shadow-xl">
              <div className="flex items-center justify-between border-b border-zinc-200 px-6 py-4">
                <div>
                  ...
                  <div className="text-sm text-zinc-600">
                    Choose courses and specific topics to provide context for
                    your AI assistant
                  </div>
                </div>
                <button
                  ...
                  {selectedCourseIds.length === 0 ? (
                    <div className="mt-4 flex h-48 items-center justify-center rounded-lg border border-zinc-200 bg-white">
                      <div className="text-center text-sm text-zinc-600">
                        ...
                        Select one or more courses to see available learning
                        topics
                      </div>
                    </div>
                  ) : (
              ...
              <div className="flex items-center justify-between gap-2 border-t border-zinc-200 px-6 py-4">
                ...
                <div className="flex items-center gap-2">
                  <Button variant="outline" onClick={() => setKbOpen(false)}>
                    Cancel
                  </Button>
            ...
            <div className="absolute left-1/2 top-24 w-full max-w-2xl -translate-x-1/2 rounded-xl bg-white shadow-xl">
              ...
              <div className="max-h-[24rem] space-y-3 overflow-y-auto px-6 py-4">
                {[
                  {
                    id: 's1',
                    title: `Create a summary of ${courses[0].name}`,
                    tag: 'summary',
                  },
                  {
                    id: 's2',
                    title: `Create a quiz about ${courses[0].name}`,
                    tag: 'quiz',
                  },
                  {
                    id: 's3',
                    title: `Generate flashcards for ${courses[0].name}`,
                    tag: 'flashcard',
                  },
                  {
                    id: 's4',
                    title: `Explain key challenges in ${courses[0].name}`,
                    tag: 'explain',
                  },
                  {
                    id: 's5',
                    title: `List best practices from ${courses[0].name}`,
                    tag: 'list',
                  },
                ].map((s) => (
                  <button
