...
function Button({ variant = 'default', size = 'default', className, ...props }: ButtonProps) {
  const base =
    'inline-flex items-center justify-center rounded-md text-sm font-medium ring-offset-white transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-zinc-900 focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50';
  const sizes = size === 'sm' ? 'h-8 px-3' : 'h-10 px-4 py-2';
  const variants =
    variant === 'outline'
...
function StudentAiPageInner() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (saved === 'default' || saved === 'academic' || saved === 'feynman' || saved === 'practitioner') {
        setLens(saved);
      }
    } catch {}
  }, []);
  ...
  async function handleRenameConversation(conversationId: string) {
    ...
    setConversationError(null);
    try {
      const updated = await updateAIConversationTitle(conversationId, renameTitle.trim());
  ...
  async function handleDeleteConversation(conversationId: string) {
    ...
    try {
      ...
      if (selectedConversationId === conversationId) router.push('/ai');
    } catch (err) {
  ...
  const [kbApplyingDescription, setKbApplyingDescription] = useState('Preparing your knowledge base and generating suggestions.');
  ...
  const [messages, setMessages] = useState<Array<{ id: string; role: 'assistant' | 'user'; content: string; pending?: boolean }>>([]);
  ...
  type GeneratedListItem = { kind: 'flashcards' | 'quiz'; id: string; title: string; count: number; createdAt: string };
  ...
  function loadConversationCache(conversationId: string): {
    ...
    try {
      ...
      const kbCourseIds = Array.isArray(parsed.kbCourseIds) ? (parsed.kbCourseIds.filter((x) => typeof x === 'string') as string[]) : [];
      const kbUnitIds = Array.isArray(parsed.kbUnitIds) ? (parsed.kbUnitIds.filter((x) => typeof x === 'string') as string[]) : [];
      ...
      const suggestions = Array.isArray(parsed.suggestions)
        ? (parsed.suggestions
            .filter(
              (s) => s && typeof s === 'object' && typeof (s as any).id === 'string' && typeof (s as any).title === 'string' && typeof (s as any).tag === 'string',
            )
            .map((s) => ({
      ...
      const askedSuggestionIds = Array.isArray(parsed.askedSuggestionIds) ? (parsed.askedSuggestionIds.filter((x) => typeof x === 'string') as string[]) : [];
  ...
  function saveConversationCache(conversationId: string, payload: { kbCourseIds: string[]; kbUnitIds: string[]; messages: any[]; suggestions: any[] }) {
    if (typeof window === 'undefined') return;
    try {
  ...
  const selectedConversationKbUnitIds = useMemo(() => (selectedConversation ? selectedConversation.knowledge_base_learning_unit_ids ?? [] : []), [selectedConversation]);

  const selectedConversationKbCourseIds = useMemo(() => (selectedConversation ? selectedConversation.knowledge_base_course_ids ?? [] : []), [selectedConversation]);

  const selectedConversationKbKey = useMemo(() => [...selectedConversationKbUnitIds].sort().join(','), [selectedConversationKbUnitIds]);

  const selectedConversationKbCourseKey = useMemo(() => [...selectedConversationKbCourseIds].sort().join(','), [selectedConversationKbCourseIds]);
  ...
  useEffect(() => {
    ...
    setKbApplied(true);
    setKbOpen(false);
  }, [selectedConversationId, selectedConversation?.id, selectedConversationKbKey, selectedConversationKbCourseKey]);
  ...
  }, [selectedConversationId, kbApplied, kbAppliedCourseIds, kbAppliedUnitIds, messages, suggestions, askedSuggestionIds]);
  ...
  const loadCourseMaterialUnits = useCallback(
    async (courseId: string) => {
      if (kbUnitsByCourse[courseId]) return;
      setKbUnitsLoadingByCourse((prev) => ({ ...prev, [courseId]: true }));
      try {
        const [units, summaries] = await Promise.all([getCourseLearningUnits(courseId, 10_000, 0), getCourseLearningUnitSummaries(courseId)]);
        const withMaterials = new Set(summaries.filter((s: LearningUnitSummary) => s.material_count > 0).map((s) => s.unit_id));
        const filtered = units.filter((u) => withMaterials.has(u.id)).sort((a, b) => a.unit_order - b.unit_order);
  ...
  }, [kbOpen, kbSelectedCourseIds, loadCourseMaterialUnits]);

  const kbOpenSnapshotRef = useRef<{ courseIds: string[]; unitIds: string[] } | null>(null);
  ...
  const toggleCourse = (id: string) => {
    setKbSelectedCourseIds((prev) => {
      ...
      if (next.includes(id)) setKbRecentlyRemovedCourseIds((removed) => removed.filter((x) => x !== id));
      return next;
    });
  };
  ...
  const toggleUnit = (id: string) => {
    setKbSelectedUnitIds((prev) => {
      const next = prev.includes(id) ? prev.filter((x) => x !== id) : [...prev, id];
      if (next.includes(id)) setKbRecentlyRemovedUnitIds((removed) => removed.filter((x) => x !== id));
      return next;
    });
  };

  const filteredCourses = kbCourses.filter((c) => c.title.toLowerCase().includes(courseQuery.toLowerCase()));
  ...
  async function refreshGenerated() {
    ...
    try {
      const [flashcards, quizzes] = await Promise.all([listGeneratedFlashcardSets(courseId), listGeneratedQuizzes(courseId)]);
      const merged: GeneratedListItem[] = [
        ...flashcards.map((s) => ({ kind: 'flashcards' as const, id: s.id, title: s.title, count: s.flashcards_count, createdAt: s.created_at })),
        ...quizzes.map((q) => ({ kind: 'quiz' as const, id: q.id, title: q.title, count: q.questions_count, createdAt: q.created_at })),
      ].sort((a, b) => {
  ...
  const filteredUnits = availableUnits.filter((u) => u.title.toLowerCase().includes(unitQuery.toLowerCase()));
  ...
  function parseSuggestions(text: string) {
    ...
    try {
      ...
      if (Array.isArray(parsed)) return parsed.filter((x) => typeof x === 'string').slice(0, 10) as string[];
    } catch {}
    const lines = trimmed
      .split('\n')
      .map((l) => l.trim())
  ...
  async function requestSuggestedQuestions(learningUnitIds: string[]) {
    const suggestionResp = await answerAI({
      query:
        'Generate 10 suggested questions a student can ask based on the selected knowledge base.\n' +
    ...
    return titles.slice(0, 10).map((t, idx) => ({ id: `s-${idx}-${Date.now()}`, title: t, tag: 'suggestion' }));
  }
  ...
  async function regenerateSuggestions() {
    ...
    try {
      ...
      setSuggestRegenerateError(err instanceof Error ? err.message : 'Failed to regenerate suggestions.');
    } finally {
  ...
  async function applyKnowledgeBase() {
    ...
    const kbChanged = removedCourseIds.length > 0 || removedUnitIds.length > 0 || addedCourseIds.length > 0 || addedUnitIds.length > 0;
    ...
    try {
      ...
      if (!kbApplied) {
        ...
        const summary = await answerAI({
          query:
            'Summarize the selected knowledge base for a student.\n\n' +
      ...
      const msg = err instanceof Error ? err.message : 'Failed to apply knowledge base.';
      setKbError(msg);
  ...
  async function handleSend() {
    ...
    if (matchedSuggestion) {
      setAskedSuggestionIds((prev) => (prev.includes(matchedSuggestion.id) ? prev : [...prev, matchedSuggestion.id]));
    }
    const now = Date.now();
    ...
    setMessages((prev) => [
      ...
      { id: pendingMsgId, role: 'assistant', content: 'Generating answer, please wait ...', pending: true },
    ]);
    try {
      const resp = await answerAI({ query: q, top_k: 10, lens, learning_unit_ids: kbAppliedUnitIds });
      setMessages((prev) => prev.map((m) => (m.id === pendingMsgId ? { ...m, content: resp.answer, pending: false } : m)));
    } catch (err) {
      const msg = err instanceof Error ? `Error: ${err.message}` : 'Error: Failed to get response.';
      setMessages((prev) => prev.map((m) => (m.id === pendingMsgId ? { ...m, content: msg, pending: false } : m)));
    } finally {
      setSending(false);
    }
  }
      ...
      <div className="flex h-full min-h-0 flex-col space-y-6 overflow-hidden">
        {!selectedConversationId ? (
                    ...
                    {renamingId === c.id ? (
                      <div className="mt-4 space-y-2" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <Button size="sm" onClick={() => void handleRenameConversation(c.id)} disabled={renameLoading}>
                            Save
                          </Button>
                        </div>
                      </div>
                      ...
                      <>
                        <div className="mt-4 text-sm font-semibold text-zinc-900">{truncateWords(c.title, 35)}</div>
                        <div className="mt-2 text-xs text-zinc-600">{formatConversationDate(c.created_at)}</div>
                      </>
                    )}
                  </div>
                ))
              ...
              <div className="fixed inset-0 z-50">
                ...
                <div className="absolute left-1/2 top-24 w-full max-w-lg -translate-x-1/2 rounded-xl bg-white shadow-xl">
                  <div className="flex items-center justify-between border-b border-zinc-200 px-6 py-4">
                    <div className="text-base font-semibold text-zinc-900">New Conversation</div>
                    <button className="rounded-md p-2 text-zinc-600 hover:bg-zinc-100" onClick={() => setCreateOpen(false)}>
                      <X className="h-4 w-4" />
                    </button>
                  </div>
                  <div className="space-y-4 px-6 py-4">
                    <Input placeholder="Conversation title" value={createTitle} onChange={(e) => setCreateTitle(e.target.value)} />
                  </div>
                  <div className="flex items-center justify-end gap-2 border-t border-zinc-200 px-6 py-4">
                    ...
                    <Button onClick={() => void handleCreateConversation()} disabled={createLoading}>
                      Create
                    </Button>
                  </div>
                </div>
          ...
          <>
            <div className="flex items-start justify-between">
              <div className="space-y-2">
                <button type="button" className="inline-flex items-center gap-2 text-sm text-zinc-700 hover:text-zinc-900" onClick={() => router.push('/ai')}>
            ...
            <div className="grid flex-1 min-h-0 grid-cols-1 gap-6 overflow-hidden lg:grid-cols-[1fr_24rem]">
              <div className="flex h-full min-h-0 flex-col space-y-4 overflow-hidden">
                <Card className="flex flex-1 min-h-0 flex-col overflow-hidden p-0" bodyClassName="flex flex-1 min-h-0 flex-col overflow-hidden p-0">
                  <div className="flex flex-1 min-h-0 flex-col">
                    <div className="flex-1 min-h-0 space-y-4 overflow-y-auto p-4">
                      {messages.length === 0 ? (
                        <div className="flex min-h-[12rem] items-center justify-center rounded-xl border border-dashed border-zinc-200 bg-white p-6 text-center text-sm text-zinc-600">
                          {!kbApplied ? 'Select a knowledge base to start chatting.' : kbApplying ? 'Preparing your knowledge base...' : 'Ask a question about your selected courses and learning units.'}
                        </div>
                      ) : (
                        messages.map((m) => (
                          <div
                            key={m.id}
                            className={m.role === 'user' ? 'grid grid-cols-[1fr_2rem] items-end gap-2' : 'grid grid-cols-[2rem_1fr] items-end gap-2'}
                          >
                            {m.role === 'assistant' && (
                              <div className="flex h-8 w-8 items-center justify-center rounded-full bg-zinc-100 text-zinc-700">AI</div>
                            )}
                            <div
                              className={`w-full rounded-2xl p-3 text-sm ${m.role === 'user' ? 'bg-zinc-900 text-white' : 'bg-zinc-100 text-zinc-900'}`}
                            >
                              {m.role === 'assistant'
                                ? m.pending
                                  ? (
                            ...
                            {m.role === 'user' && <div className="flex h-8 w-8 items-center justify-center rounded-full bg-zinc-900 text-white">U</div>}
                          </div>
                        ))
                      )}
                    ...
                    <div className="space-y-3 border-t border-zinc-200 p-3">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <Button type="button" variant="outline" className="gap-2" onClick={() => void handleSend()} disabled={!kbApplied || kbApplying || sending || !chatInput.trim()}>
                  ...
                  <div className="flex h-full flex-col space-y-4 overflow-y-auto">
                    <div>
                      ...
                      <div className="mt-3">
                        ...
                        >
                          <span>
                            {kbApplied
                              ? `${kbAppliedCourseIds.length} course${kbAppliedCourseIds.length === 1 ? '' : 's'}, ${kbAppliedUnitIds.length} unit${kbAppliedUnitIds.length === 1 ? '' : 's'} selected`
                              : 'Select Knowledge Base'}
                          </span>
                          <ChevronDown className="h-4 w-4 text-zinc-600" />
                        </button>
                        {kbChangeNotice && (
                          <div className={`mt-2 text-xs ${kbChangeNotice.toLowerCase().includes('cleared') ? 'text-red-700' : 'text-green-700'}`}>{kbChangeNotice}</div>
                        )}
                      </div>
                    </div>
                    <div className="rounded-xl border border-zinc-200 bg-white p-4">
                      ...
                      <div className="space-y-3">
                        <Button variant="outline" className="w-full" onClick={() => void handleGenerateFlashcards()} disabled={!kbApplied || kbApplying || flashcardsGenerateSubmitting}>
                          {flashcardsGenerateSubmitting ? 'Generating Flashcards...' : 'Generate Flashcards'}
                        </Button>
                        <Button variant="outline" className="w-full" onClick={handleGenerateQuiz} disabled={!kbApplied || kbApplying || quizGenerateSubmitting}>
                          Generate Quiz
                        </Button>
                      </div>
                    </div>
                    <div className="rounded-xl border border-zinc-200 bg-white p-4">
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <Button variant="outline" className="h-8 px-3" onClick={() => void refreshGenerated()} disabled={!kbApplied || kbApplying || generatedLoading}>
                          {generatedLoading ? 'Loading...' : 'Refresh'}
                        </Button>
                      </div>

                      {generatedError && (
                        <div className="mt-3 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{generatedError}</div>
                      )}

                      <div className="mt-3 max-h-72 space-y-2 overflow-y-auto pr-1">
                        {!kbApplied ? (
                          <div className="rounded-lg border border-zinc-200 bg-white p-3 text-sm text-zinc-600">Apply a knowledge base to see generated content.</div>
                        ) : generatedLoading ? (
                          <div className="rounded-lg border border-zinc-200 bg-white p-3 text-sm text-zinc-600">Loading generated content...</div>
                        ) : generatedItems.length === 0 ? (
                          <div className="rounded-lg border border-zinc-200 bg-white p-3 text-sm text-zinc-600">No generated content yet.</div>
                        ) : (
                          generatedItems.map((item) => {
                            const label = item.kind === 'flashcards' ? 'Generated Flashcards' : 'Generated Quiz';
                            const countLabel =
                            ...
                            return (
                              <button
                                ...
                                onClick={() => (item.kind === 'flashcards' ? void openGeneratedFlashcardSet(item.id) : void openGeneratedQuiz(item.id))}
                              >
                                <div className="min-w-0">
                                  <div className="text-xs text-zinc-600">{label}</div>
                                  <div className="mt-1 truncate text-sm font-semibold text-zinc-900">{item.title}</div>
        ...
        {kbOpen && (
          <div className="fixed inset-0 z-50">
            <div className="absolute inset-0 bg-black/50" onClick={closeKbModal} />
            <div className="absolute left-1/2 top-20 flex max-h-[calc(100vh-6rem)] w-full max-w-3xl -translate-x-1/2 flex-col overflow-hidden rounded-xl bg-white shadow-xl">
              <div className="flex items-center justify-between border-b border-zinc-200 px-6 py-4">
                <div>
                  ...
                  <div className="text-sm text-zinc-600">Choose courses and learning units (with materials) to provide context</div>
                </div>
                <button className="rounded-md p-2 text-zinc-600 hover:bg-zinc-100" onClick={closeKbModal}>
                  <X className="h-4 w-4" />
                </button>
              </div>
              {(kbCoursesError || kbError) && <div className="border-b border-red-200 bg-red-50 px-6 py-3 text-sm text-red-700">{kbCoursesError || kbError}</div>}
              <div className="grid min-h-0 flex-1 grid-cols-1 gap-6 overflow-hidden p-6 md:grid-cols-2">
                <div className="flex min-h-0 flex-col">
                  ...
                  <div className="mt-3 flex items-center gap-2">
                    <Button variant="outline" className="px-3 py-2" onClick={selectAllCourses} disabled={kbCoursesLoading || filteredCourses.length === 0}>
                      Select All
                    </Button>
                    <Button variant="outline" className="px-3 py-2" onClick={clearAllCourses} disabled={kbSelectedCourseIds.length === 0}>
                      Clear All
                    </Button>
                  </div>
                  <div className="mt-4 min-h-0 flex-1 space-y-3 overflow-y-auto pr-1">
                    {kbCoursesLoading ? (
                      <div className="rounded-lg border border-zinc-200 bg-white p-3 text-sm text-zinc-600">Loading courses...</div>
                    ) : filteredCourses.length === 0 ? (
                      <div className="rounded-lg border border-zinc-200 bg-white p-3 text-sm text-zinc-600">No courses found.</div>
                    ) : (
                      filteredCourses.map((c) => {
                        ...
                        const borderColor = checked ? 'border-blue-300 bg-blue-50' : recentlyRemoved ? 'border-red-200 bg-red-50' : 'border-zinc-200 bg-white';
                        return (
                          <label key={c.id} className={`flex items-start gap-3 rounded-lg border p-3 ${borderColor}`}>
                            <input type="checkbox" className="mt-1 h-4 w-4 rounded border-zinc-300" checked={checked} onChange={() => toggleCourse(c.id)} />
                            <div className="min-w-0">
                              <div className="truncate text-sm font-medium text-zinc-900">{c.title}</div>
                              <div className="text-xs text-zinc-600">
                                {kbUnitsLoadingByCourse[c.id]
                                  ? 'Loading units...'
                                  : `${(kbUnitsByCourse[c.id] ?? []).length} unit${(kbUnitsByCourse[c.id] ?? []).length === 1 ? '' : 's'} with materials`}
                              </div>
                              {recentlyRemoved && <div className="mt-1 text-xs font-medium text-red-700">Recently removed</div>}
                            </div>
                          </label>
                        );
                      })
                ...
                <div className="flex min-h-0 flex-col">
                  <div className="text-sm font-semibold text-zinc-900">Learning Units</div>
                  <div className="mt-3">
                    <Input placeholder="Search learning units..." value={unitQuery} onChange={(e) => setUnitQuery(e.target.value)} disabled={kbSelectedCourseIds.length === 0} />
                  </div>
                  <div className="mt-3 flex items-center gap-2">
                    <Button
                      variant="outline"
                    ...
                    <Button variant="outline" className="px-3 py-2" onClick={() => setKbSelectedUnitIds([])} disabled={kbSelectedUnitIds.length === 0}>
                      Clear All
                    </Button>
                  </div>
                  <div className="mt-4 min-h-0 flex-1 space-y-3 overflow-y-auto pr-1">
                    {kbSelectedCourseIds.length === 0 ? (
                      ...
                      <div className="rounded-lg border border-zinc-200 bg-white p-3 text-sm text-zinc-600">No learning units with materials found.</div>
                    ) : (
                      filteredUnits.map((u) => {
                        const courseTitle = kbCourses.find((c) => c.id === u.course_id)?.title ?? 'Course';
                        const checked = kbSelectedUnitIds.includes(u.id);
                        const recentlyRemoved = !checked && kbRecentlyRemovedUnitIds.includes(u.id);
                        const borderColor = checked ? 'border-blue-300 bg-blue-50' : recentlyRemoved ? 'border-red-200 bg-red-50' : 'border-zinc-200 bg-white';
                        return (
                          <label key={u.id} className={`flex items-start gap-3 rounded-lg border p-3 ${borderColor}`}>
                            <input type="checkbox" className="mt-1 h-4 w-4 rounded border-zinc-300" checked={checked} onChange={() => toggleUnit(u.id)} />
                            <div className="min-w-0">
                              <div className="truncate text-sm font-medium text-zinc-900">{u.title}</div>
                              <div className="truncate text-xs text-zinc-600">from {courseTitle}</div>
                              {recentlyRemoved && <div className="mt-1 text-xs font-medium text-red-700">Recently removed</div>}
                            </div>
                          </label>
                        );
              ...
              <div className="flex items-center justify-between gap-2 border-t border-zinc-200 px-6 py-4">
                <div className="text-sm text-zinc-600">
                  {kbSelectedCourseIds.length} course{kbSelectedCourseIds.length !== 1 ? 's' : ''}, {kbSelectedUnitIds.length} unit{kbSelectedUnitIds.length !== 1 ? 's' : ''} selected
                </div>
                <div className="flex items-center gap-2">
        ...
        {kbApplying && (
          <div className="fixed inset-0 z-[60]">
            ...
            <div className="absolute left-1/2 top-1/2 w-[92vw] max-w-md -translate-x-1/2 -translate-y-1/2 rounded-xl bg-white p-6 shadow-xl">
              ...
              <div className="mt-2 text-sm text-zinc-600">{kbApplyingDescription}</div>
            </div>
          </div>
        )}
        ...
        {flashcardsGenerateOpen && (
          <div className="fixed inset-0 z-50">
            ...
            <div className="absolute left-1/2 top-24 w-full max-w-2xl -translate-x-1/2 overflow-hidden rounded-xl bg-white shadow-xl">
              ...
              <div className="space-y-4 px-6 py-4">
                {flashcardsError && <div className="rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{flashcardsError}</div>}
                <div className="space-y-2">
              ...
              <div className="flex items-center justify-end gap-2 border-t border-zinc-200 px-6 py-4">
                <Button variant="outline" onClick={() => setFlashcardsGenerateOpen(false)} disabled={flashcardsGenerateSubmitting}>
                  Cancel
                </Button>
        ...
        {quizGenerateOpen && (
          <div className="fixed inset-0 z-50">
            ...
            <div className="absolute left-1/2 top-24 w-full max-w-2xl -translate-x-1/2 overflow-hidden rounded-xl bg-white shadow-xl">
              ...
              <div className="flex items-center justify-end gap-2 border-t border-zinc-200 px-6 py-4">
                <Button variant="outline" onClick={() => setQuizGenerateOpen(false)} disabled={quizGenerateSubmitting}>
                  Cancel
                </Button>
        ...
        {flashcardsOpen && (
          <div className="fixed inset-0 z-50">
            <div className="absolute inset-0 bg-black/50" onClick={() => setFlashcardsOpen(false)} />
            <div className="absolute left-1/2 top-24 w-full max-w-3xl -translate-x-1/2 overflow-hidden rounded-xl bg-white shadow-xl">
              <div className="flex items-center justify-between border-b border-zinc-200 px-6 py-4">
                ...
                <button className="rounded-md p-2 text-zinc-600 hover:bg-zinc-100" onClick={() => setFlashcardsOpen(false)}>
                  <X className="h-4 w-4" />
                </button>
              </div>
              <div className="max-h-[28rem] space-y-4 overflow-y-auto px-6 py-4">
                ...
                {flashcardsGenerating && (
                  <div className="rounded-lg border border-zinc-200 bg-white p-4">
                    <div className="flex items-center gap-3">
                      <div className="h-5 w-5 animate-spin rounded-full border-2 border-zinc-300 border-t-zinc-900" />
                      <div className="text-sm font-semibold text-zinc-900">Loading flashcards...</div>
                    </div>
                    <div className="mt-2 text-sm text-zinc-600">Fetching the saved content for this course.</div>
                  </div>
                )}

                {!flashcardsGenerating && !flashcardsError && !flashcardSet && <div className="rounded-lg border border-zinc-200 bg-white p-4 text-sm text-zinc-600">No flashcards loaded.</div>}

                {flashcardSet && (
                  <div className="space-y-4">
                    <div className="space-y-1">
                      <div className="text-lg font-semibold text-zinc-900">{flashcardSet.title}</div>
                      <div className="text-sm text-zinc-600">
                        {flashcardSet.flashcards.length} flashcard{flashcardSet.flashcards.length === 1 ? '' : 's'} • {flashcardSet.source_unit_ids.length} source unit
                    ...
                    <div className="space-y-3">
                      {flashcardSet.flashcards.map((c, idx) => {
                        ...
                        return (
                          <div key={`fc-${idx}`} className="rounded-lg border border-zinc-200 bg-white p-4">
                            <div className="flex items-start justify-between gap-3">
                              <div className="flex min-w-0 items-start gap-3">
                                <span className="mt-0.5 flex h-6 w-6 flex-none items-center justify-center rounded-full bg-zinc-900 text-xs font-semibold text-white">{idx + 1}</span>
                                <div className="min-w-0">
                                  <div className="text-sm font-semibold text-zinc-900">{c.front}</div>
                                  {revealed && <div className="mt-2 whitespace-pre-wrap text-sm text-zinc-700">{c.back}</div>}
                                </div>
                              </div>
                              <Button
                                variant="outline"
                                className="h-8 px-3"
                                onClick={() =>
                                  setRevealedFlashcardIndexes((prev) => (prev.includes(idx) ? prev.filter((x) => x !== idx) : [...prev, idx]))
                                }
                              >
                                {revealed ? 'Hide' : 'Show'}
                              </Button>
          ...
          <div className="fixed inset-0 z-50">
            ...
            <div className="absolute left-1/2 top-24 w-full max-w-3xl -translate-x-1/2 overflow-hidden rounded-xl bg-white shadow-xl">
              <div className="flex items-center justify-between border-b border-zinc-200 px-6 py-4">
                ...
                <button className="rounded-md p-2 text-zinc-600 hover:bg-zinc-100" onClick={() => setQuizOpen(false)}>
                  <X className="h-4 w-4" />
                </button>
              </div>
              <div className="max-h-[28rem] space-y-4 overflow-y-auto px-6 py-4">
                ...
                {!quizLoading && !quizError && !quiz && <div className="rounded-lg border border-zinc-200 bg-white p-4 text-sm text-zinc-600">No quiz loaded.</div>}
                ...
                {quiz && (
                  <div className="space-y-4">
                    <div className="space-y-1">
                      ...
                      <div className="text-sm text-zinc-600">
                        {quiz.questions.length} question{quiz.questions.length === 1 ? '' : 's'} • {quiz.source_unit_ids.length} source unit
                        {quiz.source_unit_ids.length === 1 ? '' : 's'}
                      </div>
                    </div>
                    ...
                    {!quizFinished ? (
                      <div className="space-y-4">
                        ...
                        {quiz.questions[quizStepIndex] ? (
                          <div className="rounded-lg border border-zinc-200 bg-white p-4">
                            <div className="text-sm font-semibold text-zinc-900">{quiz.questions[quizStepIndex].question}</div>
                              ...
                              {quiz.questions[quizStepIndex].options.map((opt, optIdx) => {
                                ...
                                return (
                                  <button
                                    ...
                                    type="button"
                                    className={`flex w-full items-start gap-3 rounded-md border px-3 py-2 text-left text-sm hover:bg-zinc-50 ${selected ? 'border-zinc-900 bg-zinc-50 text-zinc-900' : 'border-zinc-200 bg-white text-zinc-800'}`}
                                    onClick={() => setQuizSelectedOptionByIndex((prev) => ({ ...prev, [quizStepIndex]: optIdx }))}
                                  >
                                    <span className="mt-0.5 inline-flex h-5 w-5 flex-none items-center justify-center rounded bg-zinc-900 text-xs font-semibold text-white">{String.fromCharCode(65 + optIdx)}</span>
                          ...
                          <div className="rounded-lg border border-zinc-200 bg-white p-4 text-sm text-zinc-600">Question not available.</div>
                        )}

                        <div className="flex items-center justify-between gap-2">
                          <Button variant="outline" onClick={() => setQuizStepIndex((prev) => Math.max(0, prev - 1))} disabled={quizStepIndex <= 0}>
                            Previous
                          </Button>
                          <Button
                            onClick={() => {
                      ...
                      <div className="space-y-4">
                        <div className="rounded-lg border border-zinc-200 bg-white p-4">
                          ...
                          <div className="mt-2 text-sm text-zinc-600">This score is not saved.</div>
                        ...
                        <div className="space-y-3">
                          {quiz.questions.map((q, idx) => {
                            ...
                            const yourAnswer = typeof selectedOptIdx === 'number' ? q.options[selectedOptIdx] : null;
                            const correct = yourAnswer != null && yourAnswer === q.answer;

                            return (
                              <div key={`quiz-${quiz.id}-review-${idx}`} className="rounded-lg border border-zinc-200 bg-white p-4">
                                <div className="text-sm font-semibold text-zinc-900">
                                  {idx + 1}. {q.question}
                                </div>
                                <div className="mt-2 space-y-1 text-sm text-zinc-700">
                                  <div>
                                    <span className="font-semibold text-zinc-900">Your answer:</span> {yourAnswer ?? '—'}
                                  </div>
                                  <div>
                                ...
                                <div className={`mt-2 text-xs font-semibold ${correct ? 'text-green-700' : 'text-red-700'}`}>{correct ? 'Correct' : 'Incorrect'}</div>
                              </div>
                            );
                          })}
                        </div>
              ...
              <div className="flex items-center justify-between gap-2 border-t border-zinc-200 px-6 py-4">
                {quiz ? (
                  <Button variant="outline" className="border-red-200 text-red-700 hover:bg-red-50 hover:text-red-700" onClick={() => void handleDeleteGeneratedQuiz(quiz.id)} disabled={quizLoading}>
                    Delete
                  </Button>
                ) : (
                  <div />
          ...
          <div className="fixed inset-0 z-50">
            ...
            <div className="absolute left-1/2 top-24 w-full max-w-2xl -translate-x-1/2 rounded-xl bg-white shadow-xl">
              ...
              <div className="max-h-[24rem] space-y-3 overflow-y-auto px-6 py-4">
                {suggestRegenerateError && <div className="rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{suggestRegenerateError}</div>}
                {suggestRegenerating && (
                  <div className="rounded-lg border border-zinc-200 bg-white p-4">
                    <div className="flex items-center gap-3">
                      <div className="h-5 w-5 animate-spin rounded-full border-2 border-zinc-300 border-t-zinc-900" />
                    ...
                    <div className="mt-2 text-sm text-zinc-600">Generating new suggested questions for your current knowledge base.</div>
                  </div>
                )}
                {suggestions.length === 0 ? (
                  <div className="rounded-lg border border-zinc-200 bg-white p-4 text-sm text-zinc-600">No suggestions yet. Apply a knowledge base to generate them.</div>
                ) : (
                  suggestions.map((s, idx) => {
                    ...
                    return (
                      <button
                        key={s.id}
                        type="button"
                        className={`flex w-full items-start justify-between rounded-lg border p-4 text-left hover:bg-zinc-50 ${asked ? 'border-green-200 bg-green-50' : 'border-zinc-200 bg-white'}`}
                        onClick={() => {
                      ...
                      >
                        <div className="flex items-start gap-3">
                          <span className={`mt-0.5 flex h-6 w-6 flex-none items-center justify-center rounded-full text-xs font-semibold ${asked ? 'bg-green-600 text-white' : 'bg-zinc-900 text-white'}`}>
                            {idx + 1}
                          </span>
                          <div>
              ...
              <div className="flex items-center justify-between border-t border-zinc-200 px-6 py-4">
                ...
                <div className="flex items-center gap-2">
                  <Button variant="outline" onClick={() => void regenerateSuggestions()} disabled={!kbApplied || kbApplying || suggestRegenerating}>
